...
Emacs Process (Lisp)
  ...
  │     Serves /ui/index.html & pkg/* over 127.0.0.1:<port>
  │
  ├─► Workspace / Health Collector
  │     Gathers Git state, LSP status, and diagnostics
  │
...
- Serves assets exclusively from the `/ui` project folder.
- Mitigates security risks by stripping query and fragment paths, validating that paths do not traverse out of the `/ui` folder, and enforcing a strict `Cache-Control: no-store` header.
...
### 4. Workspace & Health Data Flow
- **Collector**: Emacs uses `vc-git` hooks to parse directory paths and query Git for branch status, insertions, deletions, and commits. It also inspects the viewed buffer for Eglot/lsp-mode state and Flycheck/Flymake diagnostics.
